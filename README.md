...
- `--input` — speech corpus CSV
- `--output` — where the windows are written
...
- `--input` — speech corpus CSV
- `--out-dict` — per-frame probe-word dictionary
- `--out-nouns` — corpus noun frequencies
- `--out-cosine` — category cosine-similarity matrix
...
- `--windows` — the windows from step 1
- `--output-dir` — where one model per era is saved
- `--epochs` — fine-tuning epochs per era
- Era boundaries are set by `ERA_BOUNDARIES` inside the script (must match step 4)
...
- `--windows` — the windows from step 1
- `--dict` — the dictionary from step 2
- `--nouns` — the noun counts from step 2
- `--era-models` — the models from step 3
- `--output` — where the frame scores are written
- `--top-n` — probe words averaged per frame
- Requires a GPU in practice
...
- `--scores` — the frame scores from step 4
- `--vdem` — V-Dem indicators
